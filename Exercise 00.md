...
### Функция jq (JSON Processor)
Это легкий и гибкий командный процессор для работы с данными в формате JSON. Поскольку большинство современных API возвращают данные именно в JSON, jq используется для их фильтрации и форматирования.

Форматирование (Pretty-print): Превращает нечитаемую «кашу» из текста JSON в красиво оформленную структуру с отступами и цветами.

Фильтрация: Позволяет вытащить конкретное значение из огромного массива данных (например, только ID пользователя или статус задачи).

Трансформация: Может изменять структуру данных, переименовывать поля или выполнять математические операции над ними.
Пример: curl ... | jq '.status' выведет только значение поля "status" из ответа API.
